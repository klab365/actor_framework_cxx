 ...
 * directory. Zephyr actors declared with IPC_ACTOR_DEFINE get exact-size
 * static k_thread stack storage and exact-size k_msgq backing storage.
 * The Zephyr port discovers those resources during ipc_start_all_actors().
 */
#pragma once
...
#define IPC_ACTOR_DEFINE(actor_sym, actor_name, handler_fn, stack_sz, prio, qdepth)         \
    ...
    static struct ipc_actor actor_sym = {                                                   \
        ...
        .cfg =                                                                              \
            {                                                                               \
                .stack_size  = sizeof(actor_sym##_stack),                                   \
                .priority    = (prio),                                                      \
    ...
    {                                                                                       \
        (void) ipc_port_register_static_actor_resources(                                    \
            &(actor_sym), (void *) &(actor_sym##_stack), sizeof(actor_sym##_stack),         \
            (actor_sym##_msgq_buf), sizeof(actor_sym##_msgq_buf) / sizeof(struct ipc_msg)); \
        _ipc_actor_register_static(&(actor_sym));                                           \
    }                                                                                       \
    _Static_assert((stack_sz) == sizeof(actor_sym##_stack),                                 \
                   #actor_sym ": stack_size must match emitted Zephyr stack size")
